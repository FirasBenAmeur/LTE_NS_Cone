...
/* * * * * *
 * Sana AMRI - july 2018
 * Adding the lookup table option "src/Common/LookUpTable2D.*"	 _lookUpTables
 * in the MutationppI interface "plugins/MutationppI/MutationLibrarypp.*"	
 *	 
 * To access rapidly to the modifications:  
 * ctrl + f + @modif_LkT
 * */
...
void MutationLibrarypp::defineConfigOptions(Config::OptionList& options)
{
  ...
  */
  ...
  options.addConfigOption< bool >("noElectronicEnergy","Flag telling to ignore the electronic energy.");
  ...
  options.addConfigOption< CFdouble,Config::DynamicOption<> >("FactorOmega","Factor to reduce stiffness of chemical sorce terms.");
  ...
  _lookUpTables()
{
  ...
  // = 0: default,
	// = 1: if transport properties will be
	//			computed when using the library
  _imod = 1;
  ...
  EPS = 1e-6; // this value gives problems with LTE
...
{
  CFLog(VERBOSE, "\n\nMutationLibrarypp::setup() => start\n"); 
	CFLog(NOTICE, "MutationLibrarypp::setup() => start\n"); 
  
  Framework::PhysicalChemicalLibrary::setup();
	std::cout<<">> In setup: PhysicalChemicalLibrary::setup()...\n"<<std::endl;
  
  Mutation::MixtureOptions mo(_mixtureName);
	std::cout<<">> In setup: Mixture option = "<<_mixtureName<<"\n"<<std::endl;
  mo.setStateModel(_stateModelName);
	std::cout<<">> In setup: State Model = "<<_stateModelName<<"\n"<<std::endl;
  ...
  m_hr.resize(_NS);
  m_hf.resize(_NS); 
 CFLog(NOTICE, ">> In setup: Molar mass of each species:\n");
  for (CFint i = 0; i < _NS; ++i) {
    m_molarmassp[i] = m_gasMixture->speciesMw(i);
		std::cout<<"    "<<m_molarmassp[i]<<std::endl;
  }
  ...
  CFLog(VERBOSE, "MutationLibrarypp::setup() => m_shiftHO [" << m_shiftHO << "]\n");
	CFLog(NOTICE, ">> In setup: m_shiftHO = " <<  m_shiftHO  << "\n");
  
	//P U T   H E R E    C A S E  m_shiftHO = true
  ...
  CFLog(VERBOSE, "MutationLibrarypp::setup() => end\n"); 
	CFLog(NOTICE, "MutationLibrarypp::setup() => end\n");
}
...
//////////////////////////////////////////////////////////////////////////////
//@modif_Lkp      
// NOTE:   - This function is called 476370 times during the first iter 
//					 somewhere OUTSIDE OF MutationLibrarypp.* 
//				  (Maybe in plugins/NavierStokes/NavierStokes2DPuvt.*)
...
CFdouble MutationLibrarypp::electronPressure(CFreal rhoE,CFreal tempE)
